...
transform_intensity_train = Compose(
  [
      ...
      RandScaleIntensityd(keys=['image'], factors=0.05, prob=0.09), 
  ]
)
...
transform_train = Compose(
    [
        ...
        RandRotated(
            ...
            range_x=[-np.pi*10.0/180, np.pi*10.0/180], 
            range_y=[-np.pi*25.0/180, np.pi*25.0/180],
            range_z=[-np.pi*25.0/180, np.pi*25.0/180],
            prob=0.5
        ),
        #OneOf([
            #RandGridDistortiond(
        ...
        #RandSpatialCropd(keys=['image', 'mask'], roi_size=[96, 96, 96], random_size=False),
        #SpatialPadd(keys=['image', 'mask'],  spatial_size=[96, 96, 96], method='symmetric', mode='constant', value=0)  
...
class DatasetAortaLocalizer(Dataset):
    ...
    def __getitem__(self, idx):        
        ...
        file['image'] = torch.clip((file['image'][0] - self.clip[0])/(self.clip[1] - self.clip[0]), min=0, max=1)
        file['image'] = file['image'].unsqueeze(0)
...
class Trainer:
    def __init__(self, config_name=''):
        ...
        self.entropy     = torch.nn.CrossEntropyLoss(weight=torch.tensor([1., 1., 3., 3.]).cuda().float())
    ...
    def criterion(self, outputs, masks, config):
        ...
        #config['loss_hausdorf'].append(0)

        loss = 0.05*loss_hausdorf + (1-0.05)*loss_dice
        #loss = loss_dice
    ...
    def set_net(self):
        ...
        self.net = monai.networks.nets.SegResNetDS(
            spatial_dims  = self.config_net['spatial_dims'],
            init_filters  = self.config_net['init_filters'],
            in_channels   = self.config_net['in_channels'],
            out_channels  = self.config_net['out_channels'],
            act           = self.config_net['act'],
            norm          = self.config_net['norm'],
            blocks_down   = self.config_net['blocks_down'],
            dsdepth       = self.config_net['dsdepth'],
            upsample_mode = self.config_net['upsample_mode'],
            resolution    = self.config_net['resolution']
        )
